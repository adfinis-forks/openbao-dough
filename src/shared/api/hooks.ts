...
export function useOpenBaoClient() {
  if (!globalClient) {
    throw new Error(
      'OpenBao client not initialized. Call initializeClient first.',
    );
  }
  return globalClient;
}
...
export const queryKeys = {
  ...
  secretsList: (path: string) =>
    ['secrets', 'cubbyhole', 'list', path] as const,
...
export function useCreateToken() {
  ...
  return useMutation({
    mutationFn: async (tokenData: any) => {
      const { data, error } = await client.POST('/auth/token/create', {
        body: tokenData,
      });
...
export function useRevokeToken() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      const { data: result, error } = await client.POST('/auth/token/revoke', {
        body: data,
      });
  ...
  return useMutation({
    ...
    onSuccess: (_, { path }) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['secrets', 'cubbyhole', 'list'],
      });
    },
  });
}
...
export function useCreateEntity() {
  ...
  return useMutation({
    mutationFn: async (entityData: any) => {
      const { data, error } = await client.POST('/identity/entity', {
        body: entityData,
      });
